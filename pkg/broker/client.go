...
// Client is the RabbitMQ Management HTTP API client / RabbitMQ Management HTTP API 客户端
type Client struct {
	baseURL    string
...
// HealthCheck checks whether RabbitMQ is reachable / 检查 RabbitMQ 是否可达
...
// CreateUser creates a RabbitMQ user / 创建 RabbitMQ 用户
...
// DeleteUser deletes a RabbitMQ user / 删除 RabbitMQ 用户
...
// SetPermissions sets a user's permissions on a vhost / 设置用户在 vhost 上的权限
...
// DeclareExchange declares an exchange / 声明 exchange
...
// DeclareQueue declares a queue / 声明 queue
...
// BindQueue binds a queue to an exchange / 将 queue 绑定到 exchange
...
// SetUserPassword updates an existing user's password / 更新已有用户的密码
